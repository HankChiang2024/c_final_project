...
{
    Story *pDerivedObj = (Story *)malloc(sizeof(Story));
    Scene *pObj = New_Scene(label);
    // setting derived object member
    pDerivedObj->font = al_load_ttf_font("assets/font/pirulen.ttf", 60, 0);
    pDerivedObj->background = al_load_bitmap("assets/image/story.png");
    // Load sound
    ...
    pObj->Update = Story_update;
    pObj->Draw = Story_draw;
    pObj->Destroy = Story_destroy;
    return pObj;
}
void Story_update(Scene *self)
{
    Story *Obj = ((Story *)(self->pDerivedObj));
