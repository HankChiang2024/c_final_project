...
void _Floor_load_map(Floor *floor)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0; j < 6; j++)
        {
            fscanf(data, "%d", &floor->map_data[i][j]);//載入位置點陣圖
        }
    }
    fclose(data);
}
...
void Floor_draw(Elements *self)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0; j < 6; j++)
        {
            if (Obj->map_data[i][j])
            {
                al_draw_bitmap(Obj->img, Obj->x + j * Obj->width, Obj->y + i * Obj->height, 0);//照點陣圖畫上去(圖片,位置,位置,狀態)狀態0,ALLEGRO_FLIP_VERTICAL,ALLEGRO_FLIP_HORIZONTAL
            }
        }
    }
}
